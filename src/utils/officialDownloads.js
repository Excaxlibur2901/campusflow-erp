...
const addPdfFooter = async (pdf, footerText, footerId) => {
  const pageCount = pdf.getNumberOfPages();
  const pageWidth = pdf.internal.pageSize.getWidth();
  const pageHeight = pdf.internal.pageSize.getHeight();

  // Generate real machine-readable QR code pointing to public verification URL
  const docId = footerId || 'DOC';
  const origin = typeof window !== 'undefined' && window.location?.origin ? window.location.origin : 'http://localhost:5173';
  const verificationUrl = `${origin}/verify/document/${docId}`;

  let qrDataUrl = null;
  try {
    qrDataUrl = await generateVerificationQR(verificationUrl, { width: 140 });
  } catch {
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    // Real machine-readable QR code in footer area (bottom-right)
    if (qrDataUrl) {
      try {
        const qrX = pageWidth - 14 - 20;
        const qrY = pageHeight - 30;
        pdf.addImage(qrDataUrl, 'PNG', qrX, qrY, 18, 18);
      } catch {
        // silently skip if addImage fails
      }
    }
  }
};
...
export const downloadOfficialPdf = async ({
  ...
  sections.forEach((section) => {
    ...
    (section.lines ?? []).filter(Boolean).forEach((line) => {
      y = addWrappedText(pdf, line, margin, y, maxWidth, 4);
    });
    y += 3;
  });
