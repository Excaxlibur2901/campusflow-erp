...
router.post('/', requireRole('SUPER_ADMIN', 'PRINCIPAL'), async (req, res, next) => {
  try {
    const { code, name, roomType, capacity, rowsCount, columnsCount, benchesCount, seatsPerBench, departmentId, institutionId } = req.body;
    ...
    const result = await pool.query(
      ...
      [institutionId??null, departmentId??null, code.trim().toUpperCase(), name?.trim()??code.trim(),
...
router.put('/:id', requireRole('SUPER_ADMIN', 'PRINCIPAL'), async (req, res, next) => {
  try {
    const before = await pool.query('SELECT * FROM classrooms WHERE id = $1', [req.params.id]);
